...
# MongoDB setup
client = MongoClient('mongodb+srv://beingchirag0051:Chirag5151@cluster0.e6xh9gf.mongodb.net/beingchirag0051?retryWrites=true&w=majority&appName=Cluster0')
...
wkhtmltopdf_path = r'C:\Program Files\wkhtmltopdf\bin\wkhtmltopdf.exe'  # Adjust this path according to your installation
...
def convert_html_to_pdf(html_content, output_file):
    try:
        ...
        if not os.path.exists(output_file):
            raise Exception("Error in converting HTML to PDF.")
    except Exception as e:
        logging.error("Error in convert_html_to_pdf: %s", e)
        raise
    ...
    try:
        if request.method == 'POST':
            ...
            if target_index is not None:
                ...
                final_docx_file = os.path.join(PDFS_DIRECTORY, FINAL_FILE_DOCX_FILENAME.format(unique_suffix))
                ...
                convert_html_to_pdf(html_content, final_pdf_file)
...
def view_pdf(filename):
    try:
        ...
        return render_template('view_pdfs.html', filename=filename, username=current_user.id)
    except Exception as e:
